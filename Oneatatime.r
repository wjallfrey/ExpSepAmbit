...
E11Time<-function(mu11,estimatedparas,Y,timelags){
  ...
  for(i in 1:NumberofLags){
    Tlag<-timelags[i]
    E11timemat[i,]<-normalisedgammaijTimeHAT(1,1,Tlag,Y)-normalisedtheoreticalgamma11Time(Tlag,parameters,c)
  }
  return(E11timemat)
}
loss11time<-function(mu11,estimatedparas,Y,timelags,S){
  e11<-E11Time(mu11,estimatedparas,Y,timelags)
...
S<-diag(NumberofLags)
for(i in 1:5){
  mu11hat<-optim(initialmu11,function(mu11){loss11time(mu11,estimatedparas,Y,timelags,S)})$par
  e11<-E11Time(mu11hat,estimatedparas,Y,timelags)
  omega<-e11%*%t(e11)
  S<-solve(omega)
  initialmu11<-mu11hat
}
estimatedparas$mu11<-mu11hat

E12Time<-function(mu21,estimatedparas,Y,timelags){
  NumberofLags<-length(timelags)
  c<-Y[[5]]
  Nx<-dim(Y[[1]])[2]
  ...
  for(i in 1:NumberofLags){
    Tlag<-timelags[i]
    E12timemat[i,]<-normalisedgammaijTimeHAT(1,2,Tlag,Y)-normalisedtheoreticalgamma12Time(Tlag,parameters,c)
  }
  return(E12timemat)
}
loss12time<-function(mu21,estimatedparas,Y,timelags,S){
  e12<-E12Time(mu21,estimatedparas,Y,timelags)
...
E11Space<-function(lambda11,estimatedparas,Y,spacelags){
  ...
  for(i in 1:NumberofLags){
    Llag<-spacelags[i]
    E11spaceemat[i,]<-normalisedgammaijSpaceHAT(1,1,Llag,Y)-normalisedtheoreticalgamma11Space(Llag,parameters,c)
  }
  return(E11spaceemat)
}
loss11space<-function(lambda11,estimatedparas,Y,spacelags,S){
  e11<-E11Space(lambda11,estimatedparas,Y,spacelags)
  lossfunction<-t(rowSums(e11))%*%S%*%(rowSums(e11))
  return(lossfunction)
}
initiallambda11<-1
...
E12Space<-function(lambda21,estimatedparas,Y,spacelags){
  NumberofLags<-length(spacelags)
  c<-Y[[5]]
  Nt<-dim(Y[[1]])[1]
  ...
  for(i in 1:NumberofLags){
    Llag<-spacelags[i]
    E12spaceemat[i,]<-normalisedgammaijSpaceHAT(1,2,Llag,Y)-normalisedtheoreticalgamma12Space(Llag,parameters,c)
  }
  return(E12spaceemat)
}
loss12space<-function(lambda21,estimatedparas,Y,spacelags,S){
  e12<-E12Space(lambda21,estimatedparas,Y,spacelags)
  lossfunction<-t(rowSums(e12))%*%S%*%(rowSums(e12))
  return(lossfunction)
}
initiallambda21<-1
...
CovE11Time<-function(k11,estimatedparas,Y,timelags){
  ...
  for(i in 1:length(timelags0)){
    Tlag<-timelags0[i]
    CovE11timeemat[i,]<-CijhatTime(1,1,timelags0[i],Y)-C11Time(timelags0[i],parameters,c)
  }
  return(CovE11timeemat)
}
lossCov11time<-function(k11,estimatedparas,Y,timelags,S){
  e11<-CovE11Time(k11,estimatedparas,Y,timelags)
  lossfunction<-t(rowSums(e11))%*%S%*%rowSums(e11)
  return(lossfunction)
}
initialk11<-1
...
CovE12Time<-function(k21,estimatedparas,Y,timelags){
  timelags0<-c(0,timelags) #including the zero lag as we now have covaraince
  c<-Y[[5]]
  Nx<-dim(Y[[1]])[2]
  ...
  for(i in 1:length(timelags0)){
    Tlag<-timelags0[i]
    CovE12timeemat[i,]<-CijhatTime(1,2,timelags0[i],Y)-C12Time(timelags0[i],parameters,c)
  }
  return(CovE12timeemat)
}
lossCov12time<-function(k21,estimatedparas,Y,timelags,S){
  e12<-CovE12Time(k21,estimatedparas,Y,timelags)
  lossfunction<-t(rowSums(e12))%*%S%*%rowSums(e12)
  return(lossfunction)
}
initialk21<-1
