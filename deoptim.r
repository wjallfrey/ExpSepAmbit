...
getparasM5DE<-function(Y,K){
  ...
  initialSs<-list(S11=diag(K),S12=diag(K),S22=diag(K),S11space=diag(K),S12space=diag(K),S22space=diag(K))
  datavars<-datavariogram(timelags,spacelags,Y)
  oldtheta<-quiet(DEoptim(function(theta){totallossfunction(theta,datavars,initialSs,c,timelags,spacelags)},lower=rep(0,9),upper=rep(1,9))$optim$bestmem)
  dif<-1
  while(dif>0.001){
    optimisation<-quiet(DEoptim(function(theta){totallossfunction(theta,datavars,getnewSs(oldtheta,datavars,c,timelags,spacelags),c,timelags,spacelags)},lower=rep(0,9),upper=rep(1,9))$optim)
    newtheta<-optimisation$bestmem
    dif<-max(abs(oldtheta-newtheta))
    oldtheta<-abs(newtheta)
  }
  return(newtheta)
...
paraslist04parA3expmethod<-matrix(0,100,9)
for(i in 1:100){
...
violinplot(paraslist04parA3expmethod,Y[[7]])


paraslist04parA3DE<-matrix(0,100,9)
for(i in 1:100){
  Y<-readRDS(paste("Output Fields/ambit3par04res05depth3v",i,".rds",sep=""))
  pars<-quiet(getparasM5DE(Y,10))
  paraslist04parA3DE[i,]<-pars
  if(i%%100==0){print(paste("at",i%/%10,"%",Sys.time()))}
}
violinplot(paraslist04parA3DE,Y[[7]])
